...
const formName = 'receiveCustomerPayment';
...
function ReceiveCustomerPayment(props) {
  ...
  useEffect(() => {
    ...
    return(
      ...
      <Box margin="auto" width={{ xs: '100%', md: '70%', xl: '50%' }}>
        <Typography gutterBottom variant="h6" align="center">New customer Transaction: { customer.name }</Typography>
...
const onSubmit = (values, dispatch, { match, printTxn }) => {
  ...
  return axios.post('/api/customers/receivePayment', {...match.params, ...values, time: moment(values.time, "DD MMMM, YYYY hh:mm A").toDate()}).then( response => {
