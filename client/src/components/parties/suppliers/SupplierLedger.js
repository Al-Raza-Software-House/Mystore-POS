...
import { supplierTxns } from '../../../utils/constants';
...
function SupplierLedger({ lastEndOfDay, banks, loadingRecords, dispatch }) {
 ...
 const loadTxns = useCallback(() => {
    ...
    axios.post('/api/suppliers/ledger', { storeId, supplierId, dateRange, skip, recordsPerPage: rowsPerPage} ).then( ({ data }) => {
  ...
  }, [storeId, supplierId, dispatch, records.length, rowsPerPage, dateRange]);
  ...
  const deleteTxn = useCallback((txnId) => {
    dispatch(showProgressBar());
    axios.post('/api/suppliers/deletePayment', { storeId, supplierId, txnId }).then( ({ data }) => {
      dispatch(hideProgressBar());
      dispatch( { type: accountActions.TRANSACTION_DELETED, storeId, txnId: data.accountTxnId } );
      if(data.supplier)
        dispatch( updateSupplier(storeId, supplierId, data.supplier, data.now, data.lastAction) )
      setPage(0);
      setRecords([]);
      setRecordsLoaded(false);
  ...
  return(
    <>
      <Typography gutterBottom variant="h6" align="center">{ supplier.name } - Ledger</Typography>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        ...
        <Box pt={1} minWidth="160px" display="flex" justifyContent="center">
          <Button startIcon={ <FontAwesomeIcon icon={faPlus} /> } variant="contained" color="primary" disableElevation component={Link} to={`/parties/suppliers/makepayment/${storeId}/${supplierId}`} >Pay/Receive</Button>
        </Box>
      </Box>
      {
        ...
        <Box>
          ...
          <Box width="100%" display="flex" justifyContent="space-between" alignItems="center">
            <Typography>Total Purchases: <b>{ stats.totalPurchases.toLocaleString() }</b> </Typography>
  ...
  return(
    ...
    <TableRow hover>
      ...
      <TableCell align="center">
        { txn.type === supplierTxns.SUPPLIER_TXN_TYPE_PAYMENT ? <Chip label="Payment" color="primary" /> : null }
        { txn.type === supplierTxns.SUPPLIER_TXN_TYPE_PURCHASE ? <Chip label="Purchase" /> : null }
        { txn.type === supplierTxns.SUPPLIER_TXN_TYPE_RETURN ? <Chip label="Return" variant="outlined" /> : null }
      </TableCell>
    ...
    <Popover 
        ...
        <Box py={2} px={4} textAlign="center">
          <Typography gutterBottom>Do you want to delete this entry from supplier ledger?</Typography>
