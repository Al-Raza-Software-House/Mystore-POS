...
function PointOfSale(props){
  const { storeId, store, userId, lastEndOfDay, dispatch, defaultBankId, pristine, submitting, invalid, dirty, error, handleSubmit, banks, cancelSale, printSalesReceipt, printSale, online } = props;
  ...
  }, [items, cancelSale, printSalesReceipt, printSale, customer]);
  ...
  }, [sale, dispatch, storeId, cancelSale]);
  ...
  }, [sale, dispatch, storeId, cancelSale, formValues, items]);
  if(saleId && !sale)
  {
  ...
  return(
    <form onSubmit={handleSubmit(onSubmit)}>
      <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" pb={0} style={{ paddingTop: "2px" }} >
        ...
        <Box width={{ xs: '100%', md: '24%' }}>
          <Favorites disabled={disableEdit} selectItem={selectItem} />
        </Box>
      </Box>
      
      <Box display="flex" flexDirection="column" height="100%" style={{ height:  'calc(100vh - 170px)' }}>
        ...
        <Box width="100%" textAlign="center" minHeight="18px">
          {
        ...
        <Box display="flex" justifyContent="space-between" mb={1}>
          <Box width={{xs: "100%", md: "45%"}}  display="flex" justifyContent="space-between" alignItems="center">
            <Box width="210px">
              <Adjustment disableEdit={disableEdit} totalAmountWithoutAdjustment={totalAmountWithoutAdjustment} />
            </Box>
            <Box display="flex" justifyContent="space-between" alignItems="center" flexGrow={1} ml={2}>
          ...
          <Box width={{xs: "100%", md: "53%"}} display="flex" justifyContent="space-between">
            <Box width={{xs: "100%", md: "48%"}} display="flex" justifyContent="space-between" flexWrap="wrap" pt={1}>
              <Payment {...{ pristine, sale, online, submitting, invalid, dirty, totalQuantity, totalAmount, formName, banks, storeId, handleSubmit, onSubmit, editSale, disableEdit }} />
              {
                  sale ?
            ...
            <Box width={{xs: "100%", md: "48%"}} display="flex" justifyContent={ sale ? "space-between" : "flex-end" } flexWrap="wrap" pt={1}>
              {
                !sale ? null : 
              ...
              <Button variant="contained" color="primary" type="button" disabled={items.length === 0}  startIcon={<FontAwesomeIcon icon={faTimes} />} onClick={cancelSale}>Cancel</Button>
            </Box>
          </Box>
        </Box>
      </Box>
