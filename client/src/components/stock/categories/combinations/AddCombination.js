...
function AddCombination(props){
  ...
  <>
    <ListItem button style={{ justifyContent: 'center' }} onClick={() => setOpen(true)}>
      ...
      <ListItemText style={{ flexGrow: 0 }} primary="Add Color" />
    </ListItem>
    
    <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
      <DialogTitle style={{ textAlign: 'center' }}>Add Color</DialogTitle>
...
const onSubmit = (values, dispatch, { category }) => {
  ...
  return axios.post('/api/categories/addCombination', data).then( response => {
